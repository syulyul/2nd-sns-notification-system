...
const NotificationListModalComponent = ({ notis, onReadNotiLog, onReadAllNotiLog }) => {
  return (
      <>
            ...
            {notis?.map((noti) => (
                <NotificationItem key={noti.id}>
                  <NotificationLink
                      ...
                      name={noti._id}
                      onClick={() => {
                        onReadNotiLog(noti._id); // 알림을 읽음으로 표시
                      }}
                  >
                    {noti.content}
                  </NotificationLink>
...
export default NotificationListModalComponent;
